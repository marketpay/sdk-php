...
> \MarketPay\Model\PluginPayInsResponse payInsPluginPluginClientPayInPost($plugin_pay_in)
...
try {
    $result = $apiInstance->payInsPluginPluginClientPayInPost($plugin_pay_in);
    print_r($result);
} catch (Exception $e) {
