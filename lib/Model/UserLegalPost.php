...
class UserLegalPost implements ModelInterface, ArrayAccess
{
    ...
    public function listInvalidProperties()
    {
        ...
        if (!in_array($this->container['legal_person_type'], $allowedValues)) {
        ...
        if (!in_array($this->container['legal_representative_country_of_residence'], $allowedValues)) {
            $invalidProperties[] = sprintf(
        ...
        if (!in_array($this->container['legal_representative_nationality'], $allowedValues)) {
            $invalidProperties[] = sprintf(
    ...
    {
        $allowedValues = $this->getLegalPersonTypeAllowableValues();
        if (!is_null($legal_person_type) && !in_array($legal_person_type, $allowedValues)) {
    ...
    {
        $allowedValues = $this->getLegalRepresentativeCountryOfResidenceAllowableValues();
        if (!is_null($legal_representative_country_of_residence) && !in_array($legal_representative_country_of_residence, $allowedValues)) {
            throw new \InvalidArgumentException(
                sprintf(
                    "Invalid value for 'legal_representative_country_of_residence', must be one of '%s'",
                    implode("', '", $allowedValues)
    ...
    {
        $allowedValues = $this->getLegalRepresentativeNationalityAllowableValues();
        if (!is_null($legal_representative_nationality) && !in_array($legal_representative_nationality, $allowedValues)) {
            throw new \InvalidArgumentException(
                sprintf(
                    "Invalid value for 'legal_representative_nationality', must be one of '%s'",
                    implode("', '", $allowedValues)
