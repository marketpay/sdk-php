...
class TransactionResponse implements ModelInterface, ArrayAccess
{
    ...
    public function listInvalidProperties()
    {
        ...
        if (!in_array($this->container['transaction_source'], $allowedValues)) {
            $invalidProperties[] = sprintf(
        ...
        if (!in_array($this->container['transaction_destination'], $allowedValues)) {
            $invalidProperties[] = sprintf(
    ...
    {
        $allowedValues = $this->getTransactionSourceAllowableValues();
        if (!is_null($transaction_source) && !in_array($transaction_source, $allowedValues)) {
            throw new \InvalidArgumentException(
                sprintf(
                    "Invalid value for 'transaction_source', must be one of '%s'",
                    implode("', '", $allowedValues)
    ...
    {
        $allowedValues = $this->getTransactionDestinationAllowableValues();
        if (!is_null($transaction_destination) && !in_array($transaction_destination, $allowedValues)) {
            throw new \InvalidArgumentException(
                sprintf(
                    "Invalid value for 'transaction_destination', must be one of '%s'",
                    implode("', '", $allowedValues)
